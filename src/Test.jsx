  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isNaN(measuredI) || isNaN(measuredLRA) || isNaN(measuredTP) || isNaN(measuredThresh) || isNaN(offset)) {
        ...
        console.log(loudnormData);
        return;
      }

      // === 2パス目：一度コピー（安定化用） ===
      await ffmpeg.run("-i", "input.mp3", "temp.wav");
      ...
      try {
        await ffmpeg.run("-i", "temp.wav", "-af", filterCmd, "-ar", `${sampleRate}`, "-b:a", `${bitrate}k`, "-ac", `${channel === "mono" ? 1 : 2}`, "output.mp3");
      } catch (err) {
        console.error("=== 3パス目 loudnorm 失敗 ===", err);
        console.log("ffmpegLogs:\n", ffmpegLogs.join("\n"));
        alert("3パス目 loudnorm で失敗。ログ確認してください。");
        return;
      }
    ...
    <div className="min-h-screen bg-gray-100 py-10 px-6 lg:py-16">
      <div className="lg_container">
        ...
        <p className="mt-5 mb-6 lg:mt-7 lg:mb-8">
          ...
          <br />
          1分未満の音声クリエイティブであればすぐに処理できますが、Podcast音源など数十分単位の長尺音源の場合は、
          <br className="hidden lg:block" />
          処理に時間がかかりタイムアウトする可能性があります。
          <br />
          また、無料プランのサーバーで動作しているため、処理完了までに若干時間がかかります。（目安：30秒の音源で約20〜30秒）
          <br />
